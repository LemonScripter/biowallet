...
A 12a--d.\ fázis infrastruktúrája implementálva (2026-05-25):
\texttt{dcc\_bpf\_prog\_guard} és \texttt{dcc\_task\_kill\_guard} LSM-hook-ok aktívak;
a hardveres SSH-kulcs (12b.\ fázis) FIDO2 hardware token-t igényel;
a teljes blokkoló mód (12d.\ fázis) 24 óra stabilitás után aktiválható
(lásd: \ref{sec:korlatok}.\ szakasz).
...
\begin{enumerate}[topsep=2pt,itemsep=2pt]
  ...
  \item \textit{[Részben implementálva --- 12a/c/d.\ fázis, 2026-05-25]}
        Kiterjesztett kauzális lánc távoli fizikai igazolással
        (\ref{sec:tavoli-igazolas}.\ szakasz).
        \texttt{dcc\_bpf\_prog\_guard} (LOG\_ONLY$\to$BLOCKING map-kapcsolóval)
        és \texttt{dcc\_task\_kill\_guard} (BLOCKING) aktívak;
        \texttt{remote\_token\_map} és \texttt{dcc\_rat\_daemon} telepítve.
        \textit{Hátralévő:} 12b.\ fázis hardveres FIDO2-kulcs; 12d.\ fázis
        blokkoló aktiválás (24~h stabilitási kapu); \texttt{lsm/bpf\_map}
        hook bare-metal kernelt igényel.
\end{enumerate}
