...
administration by the verified owner.  Phase~12a--d infrastructure is
implemented (2026-05-25): \texttt{dcc\_bpf\_prog\_guard} and
\texttt{dcc\_task\_kill\_guard} LSM hooks are active; hardware-bound SSH key
(Phase~12b) is pending a FIDO2 hardware token; full blocking mode
(Phase~12d) activates after 24-hour stability verification
...
\begin{enumerate}[topsep=2pt,itemsep=2pt]
  ...
  \item \textit{[Partially implemented — Phase~12a/c/d, 2026-05-25]}
        Extended causal chain with remote physical attestation
        (Section~\ref{sec:remote-attestation}).
        \texttt{dcc\_bpf\_prog\_guard} (LSM hook, LOG\_ONLY$\to$BLOCKING via
        map toggle) and \texttt{dcc\_task\_kill\_guard} (BLOCKING) are active;
        \texttt{remote\_token\_map} and \texttt{dcc\_rat\_daemon} deployed.
        \textit{Remaining:} Phase~12b hardware FIDO2 key; Phase~12d blocking
        activation (24\,h stability gate); \texttt{lsm/bpf\_map} hook requires
        bare-metal kernel.
\end{enumerate}
