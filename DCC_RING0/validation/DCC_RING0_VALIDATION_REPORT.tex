...
\begin{titlepage}
  ...
  \vspace{1.5cm}
  \begin{tabular}{ll}
    \textbf{Version:}  & v1.3 (T10/T13 + Phase~11 + Phase~12a) \\[4pt]
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Phase 12 — Extended causal chain (in progress)}
...
\begin{enumerate}[noitemsep]
  ...
  \item New BPF object \path{dcc_phase12_prog.bpf.c}: \texttt{lsm/bpf\_prog}
        hook in LOG\_ONLY mode — every BPF program operation is logged with
        the token's assurance level. Loaded and pinned at
        \path{/sys/fs/bpf/dcc_phase12_prog} (prog~ID~617).
  \item \texttt{lsm/bpf\_map} hook: compile-time OK, but BPF verifier rejects
        it on \texttt{linux-image-cloud-amd64} 6.1 (restricted kernel type
        access). Documented as cloud-kernel platform limitation;
        bare-metal deployment expected to succeed.
\end{enumerate}

\begin{notebox}
Phase~12b (TPM-bound SSH key), 12c (\texttt{dcc\_rat\_daemon}), and 12d
(blocking mode for BPF hooks) are deferred until Phase~12a has been stable
for 24~hours and the FIDO2 attestation path is verified end-to-end.
All blocking-mode activations follow the same safety protocol as Phase~11.
\end{notebox}
